<script lang="ts">
  ...
  async function handleSave(ttJson: object) {
    ...
    saving = true;
    try {
      // Convert TipTap JSON back to PmDoc then send to Rust
      // For now, save the raw Rust-format doc. The editor sends TipTap JSON
      // which we need to convert back. Since we're doing full-doc serialize
      // in Rust (Phase 3, no dirty tracking), we pass the original PmDoc.
      // TODO: convert TipTap JSON → PmDoc before saving (requires ttDocToPm helper)
      // For the initial implementation, we re-use the in-memory activeDoc.
      await invoke("save_node", { id: activeId, doc: activeDoc });
    } catch (e) {
      error = String(e);
    } finally {
      saving = false;
  ...
  onMount(async () => {
    ...
    unlisten = await listen("nodes_changed", () => loadNodes());
  });
